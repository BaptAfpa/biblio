...
<body style="background-image:url(book.png);">
    ...
    <form action="" method="GET">
        <div class=" container-fluid d-flex justify-content-evenly row ">
            <input name="firstname" class="form-control w-25 m-3" type="text"
                value="<?php print $tab[0]["first_name"] ?>" aria-label="default input example" required>
            <input name="last_name" class="form-control w-25 m-3" type="text"
                value="<?php print $tab[0]["last_name"] ?>" aria-label="default input example" required>
            <input name="phone" class="form-control w-25 m-3" type="text" value="<?php print $tab[0]["phone"] ?>"
                aria-label="default input example" required>
            <input name="email" class="form-control w-25 m-3" type="text" value="<?php print $tab[0]["email"] ?>"
                aria-label="default input example" required>
            <input name="address" class="form-control w-25 m-3" type="text" value="<?php print $tab[0]["address"] ?>"
                aria-label="default input example" required>
            <input name="city" class="form-control w-25 m-3" type="text" value="<?php print $tab[0]["city"] ?>"
                aria-label="default input example" required>
            <input name="postal_code" class="form-control w-25 m-3" type="text"
                value="<?php print $tab[0]["postal_code"] ?>" aria-label="default input example" required>
            <input name="id" type="hidden" value="<?php print $tab[0]['address_id'] ?>">
        </div>

        <div class="d-grid gap-2 col-6 mx-auto mb-5">
            <button name="modif" class="btn btn-success" type="submit">Submit</button>
        </div>
    </form>
    </div>
