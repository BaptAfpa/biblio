...
<div  >
    ...
    <div class=" container-fluid d-flex justify-content-evenly row ">
        ...
        <input name= "format" class="form-control w-25 m-3" type="text" placeholder="Format" aria-label="default input example" value="<?php print $tabDesc[0][12];?>" required>
        ...
        <input name= "quantity" class="form-control w-25 m-3" type="hidden" placeholder="Quantity" aria-label="default input example" value="<?php print $tabDesc[0]['quantity'];?>" required>
        <h1 class="text-center" style="color:white;">Authors</h1>
        <p class="text-center" style="color:red;">Leave the input empty if u wanna delete the author from this book !</p>
        <?php
        $i=0;
        ...
        ?>
        <input name= "<?php print "author".$i ;?>" class="form-control w-25 m-3" type="text" placeholder="Add a new author" aria-label="default input example">
        <?php $i++;?>
        <input name= "<?php print "author".$i ;?>" class="form-control w-25 m-3" type="text" placeholder="Add a new author" aria-label="default input example">
        <?php $i++;?>
        <input name= "<?php print "author".$i ;?>" class="form-control w-25 m-3" type="text" placeholder="Add a new author" aria-label="default input example">
        ...
        $x=0;
        ?>
        <h1 class="text-center" style="color:white;">Categories</h1>
        <p class="text-center" style="color:red;">Leave the input empty if u wanna delete the categorie from this book !</p>
        <?php
       foreach ($tabCategory as $category){
          print '<input name= "category'.$x.'" class="form-control w-25 m-3" type="text" placeholder="Category1" aria-label="default input example" value="'.$category.'" >';
          $x++;
      }
         ?>
        <input name= "<?php print "category".$x ; ?>" class="form-control w-25 m-3" type="text" placeholder="Add a category" aria-label="default input example">
        <?php $x++; ?>
        <input name= "<?php print "category".$x ; ?>" class="form-control w-25 m-3" type="text" placeholder="Add a category" aria-label="default input example"> 
        <?php $x++; ?>
        <input name= "<?php print "category".$x ; ?>" class="form-control w-25 m-3" type="text" placeholder="Add a category" aria-label="default input example"> 
