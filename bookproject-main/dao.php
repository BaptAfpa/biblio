...
class DAO
{
    ...
    public function updateQuantity($id_book)
    {

        $sql = "UPDATE book SET quantity=(quantity-1),remaining=(remaining-1) WHERE book_id='" . $id_book . "';";
    ...
    public function setRemaining($idbook = "")
    {
        $sql = "UPDATE book SET remaining=(remaining-1) WHERE book_id='" . $idbook . "';";
        return $this->executeQuery($sql);
    ...
    {
        $sql = "INSERT into rental (rental_date,book_id,return_date,customer_id,staff_id,borrow)
             values ('" . $rental_date . "','" . $book_id . "','" . $return_date . "','" . $customer_id . "','" . $staff_id . "','" . 1 . "');";
        return $this->executeQuery($sql);
    }
    ...
    {
        $sql = "SELECT rental_id, book.title, customer.last_name, customer.first_name, customer.phone, return_date, staff_id, borrow  FROM rental INNER JOIN customer ON customer.customer_id=rental.customer_id INNER JOIN book ON book.book_id=rental.book_id  ORDER BY `rental`.`return_date` ASC";
        return $this->executeQuery($sql);
    }

    public function getRentalHist($idbook = "")
    {
        $sql = "SELECT book.book_id, rental_id, book.title, customer.last_name, customer.first_name, customer.phone, staff_id, borrow,rental_date  FROM rental INNER JOIN customer ON customer.customer_id=rental.customer_id INNER JOIN book ON book.book_id=rental.book_id WHERE book.book_id='" . $idbook . "'  ORDER BY `rental`.`return_date` ASC";
        return $this->executeQuery($sql);
    }

    public function setAddRemaining($idbook = "")
    {
        $sql = "UPDATE book INNER JOIN rental ON rental.book_id=book.book_id SET remaining=(remaining+1) WHERE rental.rental_id='" . $idbook . "';";
        return $this->executeQuery($sql);
    }

    public function setAddRemainingQuantity($idbook = "")
    {
        $sql = "UPDATE book SET quantity=(quantity+1), remaining=(remaining+1) WHERE book_id='" . $idbook . "';";
        return $this->executeQuery($sql);
    }

    public function delRental($rental_id = "")
    {
        $sql = "UPDATE `rental` SET `borrow`= 0 WHERE `rental_id`='" . $rental_id . "';";
        return $this->executeQuery($sql);
    }
    ...
    public function changeBook($tabAuthor, $tabDesc, $tabCategory, $idbook)
    {
        ...
        $sql = "DELETE FROM book_author WHERE book_id='" . $idbook . "'; DELETE FROM book_category WHERE book_id='" . $idbook . "';";
        ...
        $sql15 = "UPDATE book SET title='" . addslashes($tabDesc[0]) . "',description='" . addslashes($tabDesc[7]) . "',isbn='" . addslashes($tabDesc[5]) . "',image='" . addslashes($tabDesc[8]) . "',rating='" . addslashes($tabDesc[2]) . "',rating_count='" . addslashes($tabDesc[3]) . "',pages='" . addslashes($tabDesc[4]) . "',quantity='" . addslashes($tabDesc[6]) . "',remaining='" . addslashes($tabDesc[10]) . "',format_id='" . $formatid . "' WHERE book_id='" . $idbook . "';";
        /*$sql15= "INSERT into book (title,book_id,description,isbn,image,rating,rating_count,pages,quantity,remaining,format_id)
        values ('" . addslashes($tabDesc[0]) . "','" . $idbook . "','" . addslashes($tabDesc[7]) . "','" . addslashes($tabDesc[5]) . "','" . addslashes($tabDesc[8]) . "','" . addslashes($tabDesc[2]) . "','" . addslashes($tabDesc[3]) . "','" . addslashes($tabDesc[4]) . "','" . addslashes($tabDesc[6]) . "','" .addslashes($tabDesc[10]) . "','" . $formatid. "');";
